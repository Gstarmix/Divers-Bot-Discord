...
class PlayerSetup(commands.Cog):
    ...
    async def change_channel_message_gang(self, ctx):
        await self.process_change_channel_message(ctx, "-GANG-", GARDIEN_GANG_ROLE_ID)

    @commands.command(name="y", aliases=["y1", "y2", "y3", "y4", "y5", "y6", "y7", "y8"])
    async def change_channel_message_yertirand(self, ctx):
        await self.process_change_channel_message(ctx, "Yertirand", GARDIEN_YERTI_ROLE_ID)

    async def process_change_channel_message(self, ctx, button_family, role_gardien):
        command_full = ctx.invoked_with
        canal_number = int(command_full[1:]) if command_full[1:].isdigit() else None
        if canal_number is not None:
            ...
            await ctx.send("Format de commande incorrect. Utilisez `!g<number>` ou `!y<number>` (par exemple, `!g1` ou `!y1`).")
...
def generate_message(author_name):
    embed = discord.Embed(
        ...
        description=(
            ...
            f"- Si tu as plusieurs personnages à LoL, modifie ton pseudo pour les inclure.\n"
            f"- **Première étape** : Lis les règles dans le salon <#1031609454527000616>.\n"
            f"- **Deuxième étape** : Choisis tes rôles dans le salon <#1056343806196318248>.\n"
            f"- **Troisième étape** : Choisis ta famille en cliquant sur l'un des boutons ci-dessous."
        ),
        color=0x5865f2
    )
    return embed
