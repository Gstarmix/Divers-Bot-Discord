...
class CommandCheck(commands.Cog):
    def __init__(self, bot):
        ...
        self.allowed_commands = {
            ...
            MUDAE_POKESLOT_CHANNEL_ID: ["$pokemon", "$pokedex", "$pd", "$sortpkm", "$sp", "$shinyhunt", "$sh", "$release", "$r", "$autorelease", "$arl", "$pokelike", "$pl", "$pokelikelist", "$togglepokelike", "$pokeprofile", "$pokerank", "$pokeserv", "$pokemode"],
