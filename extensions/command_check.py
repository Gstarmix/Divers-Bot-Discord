...
class CommandCheck(commands.Cog):
    def __init__(self, bot):
        ...
        self.allowed_commands[MUDAE_WAIFUS_CHANNEL_2_ID] = ["$w", "$h", "$m", "$wa", "$ha", "$ma", "$wg", "$hg", "$mg", "$us", "$tu", "$mu", "$ku", "$vote", "$daily", "$rolls", "$dk"]

        self.mod_commands = []
        for channel_id, commands in self.allowed_commands.items():
            if channel_id not in [MUDAE_MODO_CHANNEL_ID, LOG_CHANNEL_ID, MUDAE_CONTROL_CHANNEL_ID, MUDAE_WAIFUS_CHANNEL_2_ID, MUDAE_SETTINGS_CHANNEL_2_ID] + ACCOMPLISSEMENT_CHANNEL_ID:
    ...
    async def on_message(self, message):
        ...
        if command.startswith('$') or command.startswith('/'):
            if command not in self.mod_commands:
                return
            if command in self.mod_commands and (message.channel.id in [MUDAE_MODO_CHANNEL_ID, LOG_CHANNEL_ID, MUDAE_CONTROL_CHANNEL_ID, MUDAE_WAIFUS_CHANNEL_2_ID, MUDAE_SETTINGS_CHANNEL_2_ID] or message.channel.id in ACCOMPLISSEMENT_CHANNEL_ID):
                return
            allowed_channels = []
            for channel_id, commands in self.allowed_commands.items():
            ...
            if message.channel.id not in allowed_channels:
                ...
                await message.delete()
                allowed_channels_str = ', '.join([f"<#{channel_id}>" for channel_id in allowed_channels if channel_id not in [MUDAE_MODO_CHANNEL_ID, LOG_CHANNEL_ID, MUDAE_CONTROL_CHANNEL_ID, MUDAE_WAIFUS_CHANNEL_2_ID, MUDAE_SETTINGS_CHANNEL_2_ID] + ACCOMPLISSEMENT_CHANNEL_ID])
