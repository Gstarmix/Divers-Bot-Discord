        ...
        for command in message.content.split():
            if command.startswith('$') or command.startswith('/'):
                ...
                if message.channel.id not in allowed_channels and not message_sent:
                    ...
                    await message.channel.send(f"{message.author.mention} Vous avez envoyé une commande dans le mauvais salon. Veuillez l'envoyer dans le bon salon : {allowed_channels_str}.")
                    message_sent = True
                    return
