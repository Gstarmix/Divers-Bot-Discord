...
from constants import QUESTION_BOT_CHANNEL_ID, ADMIN_ROLE_ID, OTHER_ROLE_ID, AUTHOR_ROLE_ID, GSTAR_ROLE_ID
...
class Question(commands.Cog):
    ...
    async def on_thread_create(self, thread):
        ...
        if response.content.lower() == 'oui':
            ...
            # Ajouter le rôle OTHER_ROLE_ID à tous les autres membres
            other_role = thread.guild.get_role(OTHER_ROLE_ID)
            gstar_role = thread.guild.get_role(GSTAR_ROLE_ID)
            for member in thread.guild.members:
                if other_role not in member.roles and gstar_role not in member.roles:
